...
function TimeSelect({ value, min = '00:00', max = MAX_TIME, onChange }: TimeSelectProps) {
  ...
  return (
    <span className="time-select">
      <select className="time-select__part" value={h} onChange={e => handleHourChange(e.target.value)}>
        {availHours.map(hh => <option key={hh} value={hh}>{hh}</option>)}
      </select>
      <span className="time-select__sep">:</span>
      <select className="time-select__part" value={m} onChange={e => onChange(`${h}:${e.target.value}`)}>
        {availMinutes.map(mm => <option key={mm} value={mm}>{mm}</option>)}
      </select>
    </span>
  )
}
...
const sanitize = (slots: TimeSlot[]): TimeSlot[] => {
  ...
  return sorted.map((slot, i) => {
    ...
    const s = snap5(Math.max(toMin(prevEnd), Math.min(toMin(slot.startTime), toMin(nextStart) - 5)))
    const e = snap5(Math.max(s + 5, Math.min(toMin(slot.endTime), toMin(nextStart))))
...
export function ScheduleModal({ group, scenes, initialSchedule, onClose, onSave }: Props) {
  ...
  const updateSlot = (id: string, changes: Partial<TimeSlot>) =>
    setSlots(s => byStartTime(s.map(sl => sl.id === id ? { ...sl, ...changes } : sl)))
  ...
  const renderSlots = (): ReactNode[] => {
    ...
    if (toMin(slots[0].startTime) > 0) {
      items.push(
        <button key="gap-before" className="slot-add-gap"
    ...
    slots.forEach((slot, i) => {
      ...
      items.push(
        <div key={slot.id} className="slot-row">
          <TimeSelect value={slot.startTime} min={c.startMin} max={c.startMax}
            onChange={v => updateSlot(slot.id, { startTime: v })} />
          <span className="slot-arrow">→</span>
          <TimeSelect value={slot.endTime} min={c.endMin} max={c.endMax}
            onChange={v => updateSlot(slot.id, { endTime: v })} />
          <select className="slot-scene" value={slot.sceneId}
            onChange={e => handleSceneChange(slot.id, e.target.value)}>
            <option value="off">🌑 Turn off</option>
            {scenes.map(s => (
              <option key={s.id} value={s.id}>{sceneIcon(s.name)} {s.name}</option>
            ))}
          </select>
      ...
      if (toMin(slot.endTime) < toMin(gapEnd)) {
        items.push(
          <button key={`gap-${slot.id}`} className="slot-add-gap"
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal__body">
          ...
          {slots.length === 0 && (
            <button className="add-slot-btn" onClick={() => addInGap('00:00', MAX_TIME)}>
              + Add time slot
            </button>
          )}
        </div>
